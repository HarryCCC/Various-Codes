...
def add_sale():
    ...
    save_data_to_zip()
    load_data_from_zip()  # Reload the data
    update_comboboxes()
...
def update_comboboxes():
    ...
    inventory_query_entry['values'] = list(inventory.keys())
...
def query_inventory():
    item_to_query = inventory_query_entry.get()
    if item_to_query in inventory:
        inventory_query_status_label.config(text=f"库存：{inventory[item_to_query]}")
    else:
        inventory_query_status_label.config(text="该货品不存在")
...
clerk_label.grid(row=0, column=0, padx=20, pady=5)
...
clerk_entry.grid(row=0, column=1, padx=20, pady=5)

# 货品输入
item_label = ttk.Label(sales_frame, text="货品:", font=large_font)
item_label.grid(row=1, column=0, padx=20, pady=5)
...
item_sale_entry.grid(row=1, column=1, padx=20, pady=5)

# 数量输入
quantity_label = ttk.Label(sales_frame, text="数量:", font=large_font)
quantity_label.grid(row=2, column=0, padx=20, pady=5)
...
clerk_bonus_entry = ttk.Combobox(bonus_frame, width=15, values=list(clerks), font=large_font)
...
rate_entry = ttk.Entry(bonus_frame, width=15, font=large_font)
...
# 在库存查询模块里
inventory_query_frame = ttk.LabelFrame(root, text="库存查询")
inventory_query_frame.grid(row=1, column=1, padx=20, pady=5, sticky="nsew")

# 货品输入
item_inventory_query_label = ttk.Label(inventory_query_frame, text="货品:", font=large_font)
item_inventory_query_label.grid(row=0, column=0, padx=20, pady=20)
inventory_query_entry = ttk.Combobox(inventory_query_frame, width=15, values=list(inventory.keys()), font=large_font)
inventory_query_entry.bind('<KeyRelease>', auto_dropdown)
inventory_query_entry.grid(row=0, column=1, padx=20, pady=20)

# Button for Querying Inventory
query_inventory_button = tk.Button(inventory_query_frame, text="库存查询", command=query_inventory, font=large_font)
query_inventory_button.grid(row=1, columnspan=2, padx=20, pady=20)

# Status Label for Querying Inventory
inventory_query_status_label = ttk.Label(inventory_query_frame, text="------")
inventory_query_status_label.grid(row=2, columnspan=2)
