...
client_id=0SPG5iXBDpIWuDCuwrSz0pOqrd2MtHRy&
redirect_uri=oob&
scope=basic,netdisk
'''
# enapi.baidu.com/oauth/2.0/login_success#expires_in=2592000&
# access_token=123.c59cb912f1ba586f0ed4cd8bbb56e1c0.Y3VTMnQTPVROiF_dxheMP6Oo6EwEikvrbmvTa7T.3KtQRw
...
remote_file_path = "/apps/轻量级店铺会计系统/数据.zip"


# Function to upload data to Baidu Pan
def upload_to_baidu_pan(ACCESS_TOKEN, local_file_path, remote_file_path):
    upload_url = f"http://pan.baidu.com/rest/2.0/xpan/file?method=precreate&access_token={ACCESS_TOKEN}"
    params = {
        'access_token': ACCESS_TOKEN,  # 使用全局变量
        'path': remote_file_path,
        'app_id': 38981017  # 替换为你的实际应用 ID
    }
    ...
    if 'error_code' in response_json:
        print(f"Error uploading: {response_json.get('error_msg', 'Unknown error')}")
        return False
    else:
...
def add_sale():
    ...
    if upload_to_baidu_pan(ACCESS_TOKEN, local_file_path, remote_file_path):  # 如果上传成功
        sales_sync_status_label.config(text="数据已同步")
    else:
        sales_sync_status_label.config(text="数据同步失败")
...
def add_inventory():
    ...
    if upload_to_baidu_pan(ACCESS_TOKEN, local_file_path, remote_file_path):  # 如果上传成功
        ...
        inventory_sync_status_label.config(text="数据同步失败")
