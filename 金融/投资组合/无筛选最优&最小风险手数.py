...
indices_selected_stocks_orp = np.where(weights_sharpe_max > 0.001)[0]
...
indices_selected_stocks_mvp = np.where(weights_std_min > 0.001)[0]
...
with open("投资组合手数信息.txt", "w") as file:
    file.write("Optimal Risky Portfolio (ORP):\n")
    for name_code, weight, closing_price, hands, adjusted_hands in zip(selected_stock_names_codes_orp, selected_weights_sharpe_max, selected_closing_prices_orp, hands_orp, adjusted_hands_orp):
        file.write(f"{name_code}: Weight={weight*100:.2f}%, Closing Price={closing_price:.2f}, Adjusted Hands={adjusted_hands:.2f}\n")
    
    file.write("\nMinimum Variance Portfolio (MVP):\n")
    for name_code, weight, closing_price, hands, adjusted_hands in zip(selected_stock_names_codes_mvp, selected_weights_std_min, selected_closing_prices_mvp, hands_mvp, adjusted_hands_mvp):
        file.write(f"{name_code}: Weight={weight*100:.2f}%, Closing Price={closing_price:.2f}, Adjusted Hands={adjusted_hands:.2f}\n")
