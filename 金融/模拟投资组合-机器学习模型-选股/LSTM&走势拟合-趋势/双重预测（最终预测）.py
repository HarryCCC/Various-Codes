...
for i, (code, name, predict_data) in enumerate(zip(stock_codes, stock_names, df_predict.values)):
    ...
    pattern_results.append((code, name, avg_total_return, list(zip(top_10_distances, top_10_returns))))

# 根据平均总收益率从大到小排序
pattern_results.sort(key=lambda x: x[2], reverse=True)

# 将预测结果保存为字典
pattern_dict = {stock[0]: stock[2] / 100 for stock in pattern_results}
...
top_100_results = sorted_scores[:500]
output_data = []
for code, score in top_100_results:
    name = stock_names[list(stock_codes).index(code)]
    prediction = predict_dict[code]
    pattern = pattern_dict[code]
    output_data.append([code, name, prediction, pattern, score])

output_df = pd.DataFrame(output_data, columns=['股票代码', '股票名称', '预测周度收益率', '历史模式预测收益率', '综合得分'])
output_df.to_excel('LSTM&走势拟合-预测结果.xlsx', index=False)
