...
def extract_text(image_path, min_area_threshold=10):
    ...
    # 创建一个与原图大小相同的白色背景
    filled_text = np.ones_like(image) * 255
    ...
    for contour in contours:
        ...
        if area >= min_area_threshold:
            ...
            seed_point = tuple(contour[0][0])  # 选择轮廓上的一个点作为种子点
            cv2.floodFill(filled_text, None, seed_point, (0, 0, 0))
    
    # 将填充后的文字图像转换为透明背景的 PNG 图像
    bgra = cv2.cvtColor(filled_text, cv2.COLOR_BGR2BGRA)
    bgra[:, :, 3] = np.where(filled_text[:, :, 0] == 255, 0, 255)
